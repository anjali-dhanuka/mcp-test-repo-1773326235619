import { RepositoryParamsSchema, ListRepositoriesParamsSchema, CreateRepositoryParamsSchema, ListContentsParamsSchema } from '../types/mcp.js';
...
export class RepositoryTools {
    ...
    async listRepositories(params) {
        try {
            ...
            return createSuccessResult(repositories.map(repo => ({
                id: repo.id,
                name: repo.name,
     ...
     */
    async getRepositoryCloneUrls(params) {
        try {
            const { owner, repo } = validateParams(RepositoryParamsSchema, params);
            validateGitHubParams({ owner, repo });
            const repository = await this.client.getRepository(owner, repo);
     ...
     */
    async listRepositoryContents(params) {
        try {
            ...
            const contents = await this.client.getFileContent(validatedParams.owner, validatedParams.repo, validatedParams.path || '', validatedParams.ref);
            // Handle both single file and directory listing
            const items = Array.isArray(contents) ? contents : [contents];
            return createSuccessResult(items.map(item => ({
                name: item.name,
                path: item.path,
                type: item.type,
                size: item.size,
    ...
    async listUserRepositories(params) {
        try {
            const validatedParams = validateParams(ListRepositoriesParamsSchema.extend({
                username: require('zod').string().describe('GitHub username')
            }), params);
            validateGitHubParams({ username: validatedParams.username });
            const repositories = await this.client.listUserRepositories(validatedParams.username, {
     ...
     */
    async listOrganizationRepositories(params) {
        try {
            const validatedParams = validateParams(ListRepositoriesParamsSchema.extend({
                org: require('zod').string().describe('Organization name')
            }), params);
            validateGitHubParams({ org: validatedParams.org });
            const repositories = await this.client.listOrganizationRepositories(validatedParams.org, {
                type: validatedParams.type,
                sort: validatedParams.sort,
                direction: validatedParams.direction,
                per_page: validatedParams.per_page,
